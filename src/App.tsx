...
export default class App extends Component<Props, State> {
  ...
  componentDidMount() {
    ...
    for (let i = 0; i < 50; i++) {
      member = {
        ...
        oldDirectionX: 0,
        oldDirectionY: 0,
        status: i < 1 ? statusPerson.infected : statusPerson.none,
      };
      if (i < 1) {
  ...
  draw() {
    ...
    this.state.arrayMembers.map((member: person, index: number) => {
      ...
      if (
        ...
        member.status === statusPerson.none
      ) {
        statusInfected = statusPerson.infected;
      }
  ...
  _infection(person: person, index: number) {
    ...
    if (isInfected) {
      ...
      arrayMembers[index].status = statusPerson.infected;
  ...
  _limitCanvas(member: person, statusInfected: statusPerson, index: number) {
